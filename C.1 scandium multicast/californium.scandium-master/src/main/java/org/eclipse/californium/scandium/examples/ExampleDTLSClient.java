...
public class ExampleDTLSClient {
	...
	public void test() {
		try {
			...
			InetSocketAddress servAdd = new InetSocketAddress("192.168.1.137" ,0);
			
			dtlsConnector.send(new RawData("REQJOINGROUP".getBytes(), servAdd.getAddress() , DEFAULT_PORT));
			//dtlsConnector.send(new RawData("REQJOINGROUP".getBytes(),InetAddress.getByName("localhost") , DEFAULT_PORT));
