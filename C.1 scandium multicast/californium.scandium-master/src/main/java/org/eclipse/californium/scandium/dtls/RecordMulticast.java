...
public class RecordMulticast {

	// Logging ////////////////////////////////////////////////////////

	protected static final Logger LOGGER = Logger.getLogger(RecordMulticast.class.getCanonicalName());
	...
	private static final int MULTICASTID_BITS = 8;
	
	private static final int SEQUENCE_NUMBER_BITS = 40;
	...
	/** A multicasting ID for multicasting messages **/
	 ...
	 * by the type field.
	 */
	private DTLSMulticastMessage fragment = null;

	/** The raw byte representation of the fragment. */
	private byte[] fragmentBytes = null;

	/** The DTLS session. */
	private DTLSMulticastSession session;
	 ...
	 */
	public RecordMulticast(ContentType type, ProtocolVersion version, int epoch, int multicastid, long sequenceNumber, int length, byte[] fragmentBytes) {
		this.type = type;
	...
	public RecordMulticast(ContentType type, int epoch, int multicastid, int sequenceNumber, DTLSMulticastMessage closeNotify, DTLSMulticastSession session) {
		this.type = type;
		this.epoch = epoch;
		this.multicastid = multicastid;
		this.sequenceNumber = sequenceNumber;
		this.session = session;
		setFragment(closeNotify);
	}
	...
	public byte[] toByteArray() {
		...
		writer.write(multicastid, MULTICASTID_BITS);
	 ...
	 */
	public static List<RecordMulticast> fromByteArray(byte[] byteArray) {
		List<RecordMulticast> records = new ArrayList<RecordMulticast>();
		...
		while (reader.bytesAvailable()) {
			...
			int multicastid = reader.read(MULTICASTID_BITS);
			...
			records.add(new RecordMulticast(contentType, version, epoch, multicastid,sequenceNumber, length, fragmentBytes));
		}
	...
	public DTLSMulticastSession getSession() {
		return session;
	}

	public void setSession(DTLSMulticastSession session2) {
	 ...
	 *             if the decryption fails.
	 */
	public DTLSMulticastMessage getFragment() throws HandshakeException {
		if (fragment == null) {
			...
			case APPLICATION_DATA:
				...
				if (decryptedMessage != null) {
					fragment = ApplicationMessageMulticast.fromByteArray(decryptedMessage);
				}
				break;
			default:
	 ...
	 *            the DTLS fragment.
	 */
	public void setFragment(DTLSMulticastMessage fragment) {
	...
	public String toString() {
		...
		sb.append("Multicast ID" + multicastid + "\n");
